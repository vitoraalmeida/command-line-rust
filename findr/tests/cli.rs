...
// --------------------------------------------------
// define como ler o nome do arquivo em sistemas windows
#[cfg(windows)]
...
fn run(args: &[&str], expected_file: &str) -> TestResult {
    // lê o arquivo de forma diferente a depender do SO
    let file = format_file_name(expected_file);
    let contents = fs::read_to_string(file.as_ref())?;
    // coleta as linhas que não estiverem vazias
    let mut expected: Vec<&str> =
        contents.split("\n").filter(|s| !s.is_empty()).collect();
    // ordena as linhas para que ordem de saída seja indiferente
    // pois o resultado também será ordenado e comparado com o esperado
    expected.sort();

    let cmd = Command::cargo_bin(PRG)?.args(args).assert().success();
    let out = cmd.get_output();
    let stdout = String::from_utf8(out.stdout.clone())?;
    // ordena também o resultado para comparar com o esperado
